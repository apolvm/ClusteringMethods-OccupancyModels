...
doublingNumberSites <- function(myFrame, nSites, nVisits, yDataFrame, dateDataFrame){
  nSites2<- nSites*2
  nVisits2 <- nVisits/2
  #-sites-
  sitesDF <- 1:(nSites2)
  ...
  ## merging Y's together
  yDF <- data.frame(matrix(NA, nrow=nSites2, ncol = nVisits2))
  i=1
  nVisit2 <- nVisits2+1
  while( i <= (nVisits2)){
    yData <- matrix(data=c(yDataFrame[ ,i], yDataFrame[, nVisit2]))
    yDF[,i] <- yData[, 1]
    i <- i+1
    nVisit2<- nVisit2+1
    
  }
  ##merging dates together
  dateDF <- data.frame(matrix(NA, nrow=nSites*2, ncol = nVisits/2))
  nVisits2 <- nVisits/2
  i=1
  nVisits2 <- nVisits2+1
  while( i <= (nVisits/ 2)){
    dateData <- matrix(data=c(dateDataFrame[ ,i], dateDataFrame[, nVisits2]))
    dateDF[, i] <- dateData[, 1]
    i <- i+1
    nVisits2<- nVisits2+1
  }
  
  ##doubling temp values
  tempNum <- which( colnames(myFrame) == "temp")
  tempDF <- matrix(data= NA, nrow = nSites * 2, ncol=1)
  i= 1
  k= nSites+1
  while(i <= nSites){
    tempDF[i, ] <- myFrame[i,tempNum]
    tempDF[k, ] <- myFrame[i,tempNum]
    i = i+1
    k= k+1
  }
  tempDF <- data.frame(tempDF)
  
  ##doubling elevation values
  elevNum <- which( colnames(myFrame) == "elev")
  elevDF <- matrix(data= NA, nrow = nSites * 2, ncol=1)
  i= 1
  k= nSites+1
  while(i <= nSites){
    elevDF[i, ] <- myFrame[i,elevNum]
    elevDF[k, ] <- myFrame[i,elevNum]
    i = i+1
    k= k+1
  }
  elevDF <- data.frame(elevDF)
  ...
  ##Creating dataframe columns
  cols <- c("site")
  myFrame2 <- data.frame(matrix(NA, nrow= nSites*2 , ncol = nVisits/2))
  for (i in 1: (nVisits/2)) {
    cols[i+1] <- addString("y.",i)
  }
  ...
  myFrame2 <- cbind(sitesDF, yDF, elevDF, tempDF, dateDF)
  colnames(myFrame2)<- cols
  yDataFrame <- data.frame(yDF)
  dateDataFrame <- data.frame(dateDF)
}
...
for(i in 1: 10){
  ...
  dateDataFrame <- data.frame( "date"= dates)
  
  myFrame2<- doublingNumberSites(myFrame, nSites, nVisits, yDataFrame, datesDataFrame)
